...
Max6651ClosedLoop::Max6651ClosedLoop() : i2c_(0),
                                         address_(0),
                                         mode_config_(),
...
Max6651ClosedLoop::Max6651ClosedLoop(TeensyI2CMaster *i2c, uint8_t address,
...
int Max6651ClosedLoop::targetSpeed(int rpm)
{
    ...
    else
    {
        if (stopped())
        {
            Serial.println("fan_stopped - fan_run");
            run();
        }
    }
...
int Max6651ClosedLoop::calibrateMinimumSpeed(uint8_t seconds, uint8_t index)
{
    ...
    Serial.printf("index %d minspeed: %d\n", index, measured_rpm);
    return kTachToSpeed(k_tach_min_speed_);
}
...
int Max6651ClosedLoop::calibrateMaximumSpeed(uint8_t seconds, uint8_t index)
{
    ...
    Serial.printf("index %d maxspeed: %d\n", index, measured_rpm);
    return kTachToSpeed(k_tach_max_speed_);
}
